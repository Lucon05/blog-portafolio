...
function backendErrorHandler(res: Response, body: unknown) {
  // manejo de errores   // se usa en varios lugares
  if (!res.ok) {
    // si algo no esta bien  revisa el body toma ese error y lo imprime en la consola como "Error desconocido"
    try {
...
export async function fetchPost(slug: string) {
  // esto toma la informacion  del slug en la BD y sela pasa al frontend
...
export async function createPost(postData: {
  // crear post:  se usa en new-post.tsx tambien  porque tiene una logica parecida en edit-post.tsx
  title: string;
  body: string;
  published: boolean;
}) {
...
export async function putPost( // editar post
  ...
  const res = await fetch("/api/posts/" + slug, {
    //sellama desde exactamente ese slug a la api y usamos el methodo put que es la encargada de editar
    ...
    headers: {
      //mas que editar lo que hace es actualizar los valores de la ta bla en BD
...
export async function deletePost(slug: string) {
  //eliminar post
  const res = await fetch("/api/posts/" + slug, {
    //se  llama desde exactamente ese  slug a la api y usamos el methodo delete eliminar datos
    method: "DELETE",
  });
